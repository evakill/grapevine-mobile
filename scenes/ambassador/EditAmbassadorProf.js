...
import { StyleSheet, Text, Button, TextInput, ScrollView, Dimensions } from 'react-native';
...
export default class EditAmbassadorProf extends React.Component {
  constructor(props) {
    super(props);
    const { navigation } = this.props;
    this.state = navigation.getParam('ambassador', '');
  }

  componentDidMount(){
    const self = this;
    socket.on('new ambassador profile', function(ambassador){
      self.props.navigation.navigate('AmbassadorProf', {ambassador: ambassador});
    });
  }
        ...
        <ScrollView
          style={{flex:1}}>
            <View style={styles.form}>
              ...
              />
              <FormLabel labelStyle={styles.h2}>Address</FormLabel>

            <GooglePlacesAutocomplete
              placeholder={this.state.address || 'enter an adress'}
              minLength={2}
              autoFocus={false}
              returnKeyType={'default'}
              fetchDetails={true}
              styles={{
                container: {backgroundColor:c.gray1, borderWidth:0, marginBottom:15, alignSelf: "center"},
            ...
            />
              ...
              <ButtonGroup
                onPress={(i) => {i===0 ? socket.emit('edit ambassador profile', this.state) : this.props.navigation.navigate('AmbassadorProf')}}
                buttons={['Save', 'Cancel']}
